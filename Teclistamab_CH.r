...
#-- [25-11-17] Tecli data 변경--#
tecli_new <-read_excel('C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/Data/teclistamab_data_weight_height_2025-11-12.xlsx', sheet = 1) %>%  dplyr::select(ISS_DX, SEX, CTX_S_DATE, BIRTH, CTX_LINE, HB_CTX, PLT_CTX, GFR_CTX, LDH_HIGH1_CTX, VS_PFS_CTX, DATE_PFS_CTX, PRE_CTX_R, VS, DATE_LAST, RESP_CTX, PID, DATE_DX, LDH_CTX, CREAT_CTX, ANC_CTX, ALC_CTX, Heavychain, Lightchain, FGFR3_IGH_DX_VALUE, MAF_IGH_DX_VALUE, CCND1_IGH_DX_VALUE, RB1_DX_VALUE, TP53_DX_VALUE, `1Q_DX`)%>% 
  mutate(CTX_S_DATE = as.Date(CTX_S_DATE),
         BIRTH = as.Date(BIRTH),
         DATE_PFS_CTX = as.Date(DATE_PFS_CTX),
         DATE_LAST = as.Date(DATE_LAST),
...
data_SOC <- data_tecli[data_tecli$Group==2,]
nrow(data_SOC); nrow(tecli_new) # 90, 30
tecli_new$Group = rep(1,nrow(tecli_new))
length(tecli_new$Group) # 30

data_new = rbind.data.frame(data_SOC, tecli_new)
nrow(data_new) # 120
data_new$Group

data_tidy <- data_new %>% 
  mutate(CTX_LINE_match3 = case_when(
    CTX_LINE == 1 ~ "3",
    CTX_LINE == 2 ~ "3",
    CTX_LINE == 3 ~ "3",
    CTX_LINE == 4 ~ "4",
    ...
    CTX_LINE >= 10 ~ "6",
    TRUE ~ NA_character_))  %>%
  mutate(CTX_LINE_match4 = case_when(
    CTX_LINE == 1 ~ "4",
    CTX_LINE == 2 ~ "4",
    CTX_LINE == 3 ~ "4",
    CTX_LINE == 4 ~ "4",
...
data_tidy %>%
  ...
  flextable::save_as_docx(
    ...
    path = "C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/Baseline/baseline_Unmatched.docx"
  )

data_tidy$Group <- factor(data_tidy$Group, levels = c("SOC", "Teclistamab")) #soc에 비해
...
setwd("C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/[25-09-08] Figure")
...
data_tidy %>% dplyr::select(Group, RESP_CTX) %>%
  ...
  dplyr::select(Group, n, CR, PR, SD, PD, ORR, DCR)
...
# 옛날 코드
data_tecli %>% filter(PID %in% matched_data$PID) %>% dplyr::select(Group, AGE) %>%
  mutate(AGE = ifelse(AGE < 65, "65", "over 65")) %>%
  tbl_summary(Group)

matched_data %>%
  dplyr::select(Group, ISS_DX, SEX, CTX_LINE, HB_CTX, PLT_CTX, GFR_CTX, AGE, LDH_HIGH1_CTX) %>%
  tbl_summary(Group, 
...
matched_data$PLT_CTX
matched_data$CTX_LINE
matched_data$AGE
...
exp_form = Group~ AGE + PLT_CTX + CTX_LINE
...
col_val = NULL
col_lab = NULL
var_lab = c("Line of therapy","Platelet count <50 x 10^9/L", "Age")
...
dev.new()
pdf("C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/[25-09-08] Figure/250908_smdplot.pdf",height=7,width=12)
tab_smd_res %>%
...
#! Baseline ---------------------------------------------------------------
matched_data$Group <- factor(matched_data$Group, levels = c("Teclistamab", "SOC"))
...
matched_data %>%
  ...
  flextable::save_as_docx(
    "Matched covariates" = .,
    path = "C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/Baseline/baseline_Matched.docx"
  )

matched_data$Group <- factor(matched_data$Group, levels = c("SOC", "Teclistamab")) #soc에 비해
...
setwd("C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/[25-09-08] Figure")
fit <- survfit2(Surv(TIME2/30.42, VS) ~ Group, data = matched_data)
cox_fit <- coxph(Surv(TIME2 / 365, VS) ~ Group + cluster(subclass),data = matched_data)
summary(cox_fit)
# opt_plot(fit, name="image_OS_psm", deadline=2, conf = FALSE, ptitle = "up", ms = TRUE, cox = cox_fit, col = "")

# Fit survival model (PFS)
fit <- survfit2(Surv(TIME/30.42, VS_PFS_CTX) ~ Group, data = matched_data)
cox_fit <- coxph(Surv(TIME / 365, VS_PFS_CTX) ~ Group + cluster(subclass), data = matched_data)
summary(cox_fit)
# opt_plot(fit, name="image_PFS_psm", deadline=1.5, conf = FALSE, ptitle = "up", ms = TRUE, cox = cox_fit, col = "")


# Median follow-up survival ---------------------------------------------------------------
median_fw_surv_dat <- copy(matched_data)
median_fw_surv_dat$VS <- ifelse(median_fw_surv_dat$VS==0, 1, 0)
median_fw_surv_dat$VS_PFS_CTX <- ifelse(median_fw_surv_dat$VS_PFS_CTX==0, 1, 0)
...
matched_data %>% dplyr::select(Group, RESP_CTX) %>% 
  group_by(Group) %>%
  summarise(
    n = n(),
    CR = sum(RESP_CTX == "CR", na.rm = TRUE),
  ...
  dplyr::select(Group, n, CR, sCR, PR, SD, PD, ORR, DCR)
...
# OS
matched_data_land7 <- matched_data[matched_data$TIME2 >= 7,]
matched_data_land30 <- matched_data[matched_data$TIME2 >= 30,]
...
# PFS
matched_data_land7 <- matched_data[matched_data$TIME >= 7,]
matched_data_land30 <- matched_data[matched_data$TIME >= 30,]
...
# 6) 호출
# png("C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/[25-09-08] Figure/OS_forest_plot.png", width = 1000, height = 900, res = 220)
dev.new()
pdf("C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/[25-09-08] Figure/OS_forest_plot.pdf",height=7,width=12)
...
# 6) 호출
dev.new()
pdf("C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/논문 작업/1st/[25-09-08] Figure/PFS_forest_plot.pdf",height=7,width=12)
p <- opt_plot2(rowlist, model, level)   # 필요하면 col="rev"
dev.off()
...
#! Exposure analysis
exposure <- read_excel('C:/Users/chaehyun/Dropbox/Work/PIPET/과제/혈액내과/Teclistamab/Data/3~10차_chemo_2025-07-18_투여일_AE완료_simulation결과수정.xlsx', sheet = 1, skip = 1) %>% select(Group, starts_with('AUC'), starts_with('CMAX'), starts_with('AMT'), `Ctrough(last obs-one wk)`, PID, TEC_NO) %>% filter(Group == 1, TEC_NO >= 5) %>% select(-Group)
exposure
colnames(exposure)
